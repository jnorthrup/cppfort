...
janus: @enum type = {
    ...
    void flip(inout this) = {
        if this == past { this = future; }
        else { this = past; }
    }
}auto file_attributes: @flag_enum<u8> type = {
    cached;     // 1
    current;    // 2
    obsolete;   // 4
...
int main() { auto j = janus::past;
    j.flip();
    _ = j;

    skat_game // x = 9;               auto // error, can't construct skat_game from integer
    ...
    // x = rgb::red;                    auto // error, can't assign skat_game from rgb color
