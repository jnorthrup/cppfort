...
void test(forward v) { if v is (42) {
        std::cout << " 42";
    }
    if v is (24) {
        std::cout << " 24";
...
int main() { std::variant<std::monostate, int, int, std::optional<int>, std::any, *int, std::unique_ptr<int>, my_variant> v = ();
...
void header(const int& lvl, const std::string& msg) { std::cout << std::string(lvl, '#') << " " << msg << std::endl; }
