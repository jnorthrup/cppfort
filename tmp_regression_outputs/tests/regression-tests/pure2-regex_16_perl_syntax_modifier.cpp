#include <iostream>
#include <string>
std::string create_result(std::string resultExpr, r) { std::string result = "";
  ...
  auto extract_group_and_advance = auto :(inout iter) -> _ = {
    start = iter;
  ...
  auto extract_until = auto :(inout iter, to: char) -> _ = {
    start = iter;
  ...
  auto while iter != resultExpr.end() {
    next = get_next(iter);
...
std::string sanitize(std::string str) { str = cpp2::string_util::replace_all(str, "\a", "\\a");
...
test:auto <M> (regex: M, id: std::string, regex_str: std::string, str: std::string, kind: std::string, resultExpr: std::string,
...
}void test_tests_16_perl_syntax_modifier: @regex type = {
  regex_01 := R"(/\N {1}/x)";
  regex_02 := R"(/\N {1}/x)";
  regex_03 := R"(/\N {1}/x)";
  regex_04 := R"(/\N {3,4}/x)";
  ...
  regex_32 := R"(/[#]/x)";
  regex_33 := R"(/[#]b/x)";
  run(this) { std::cout << "Running tests_16_perl_syntax_modifier:"<< std::endl;
...
int main() { test_tests_16_perl_syntax_modifier().run(); }
