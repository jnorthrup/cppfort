...
test:auto <M> (regex: M, id: std::string, regex_str: std::string, str: std::string, kind: std::string, resultExpr: std::string,
...
}void test_tests_14_multiline_modifier: @regex type = {
  regex_01 := R"(\Z)";
  regex_02 := R"(\z)";
  regex_03 := R"($)";
  regex_04 := R"(\Z)";
  ...
  regex_198 := R"('abb$'m)";
  regex_199 := R"('\Aa$'m)";
  run(this) { std::cout << "Running tests_14_multiline_modifier:"<< std::endl;
...
int main() { test_tests_14_multiline_modifier().run(); }
