...
X: type = {
    ...
    operator=(out this, cpp2::impl::out<Y> y) { //  === The following comments will stay close to, but not exactly at,
...
using //  Another type Y, that uses X
//
Y = {
    px: *X;
...
namespace M = { using //  Testing nested templated = {
    B: <I: int> type = {
...
void main() { N::Y y;            N::X // declare an uninitialized Y object
    x = (out y);  // construct y and x, and point them at each other
