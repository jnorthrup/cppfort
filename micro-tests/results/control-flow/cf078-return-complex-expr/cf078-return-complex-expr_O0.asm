
/Users/jim/work/cppfort/micro-tests/results/control-flow/cf078-return-complex-expr/cf078-return-complex-expr_O0.out:	file format mach-o arm64

Disassembly of section __TEXT,__text:

0000000100000360 <__Z7computeii>:
100000360: d10043ff    	sub	sp, sp, #0x10
100000364: b9000fe0    	str	w0, [sp, #0xc]
100000368: b9000be1    	str	w1, [sp, #0x8]
10000036c: b9400fe8    	ldr	w8, [sp, #0xc]
100000370: b9400be9    	ldr	w9, [sp, #0x8]
100000374: 0b090108    	add	w8, w8, w9
100000378: b9400fe9    	ldr	w9, [sp, #0xc]
10000037c: b9400bea    	ldr	w10, [sp, #0x8]
100000380: 6b0a0129    	subs	w9, w9, w10
100000384: 1b097d08    	mul	w8, w8, w9
100000388: b9400fe9    	ldr	w9, [sp, #0xc]
10000038c: b9400bea    	ldr	w10, [sp, #0x8]
100000390: 1b0a7d29    	mul	w9, w9, w10
100000394: 0b090100    	add	w0, w8, w9
100000398: 910043ff    	add	sp, sp, #0x10
10000039c: d65f03c0    	ret

00000001000003a0 <_main>:
1000003a0: d10083ff    	sub	sp, sp, #0x20
1000003a4: a9017bfd    	stp	x29, x30, [sp, #0x10]
1000003a8: 910043fd    	add	x29, sp, #0x10
1000003ac: b81fc3bf    	stur	wzr, [x29, #-0x4]
1000003b0: 528000a0    	mov	w0, #0x5                ; =5
1000003b4: 52800061    	mov	w1, #0x3                ; =3
1000003b8: 97ffffea    	bl	0x100000360 <__Z7computeii>
1000003bc: a9417bfd    	ldp	x29, x30, [sp, #0x10]
1000003c0: 910083ff    	add	sp, sp, #0x20
1000003c4: d65f03c0    	ret
