...
> Note: `is` unifies a variety of differently-named Cpp1 language and library queries under one syntax, and supports only the type-safe ones.
    ...
    Note: In Cpp2 we distinguish between *pure concepts* and *metafunctions*: a pure concept is a compile-time predicate (e.g., `std::integral`) that can be used in `is` and `requires` constraints, but it does not generate code or apply defaults; a metafunction (applied with `@`) can both assert constraints and — in the general case — transform a declaration and generate additional code. `is` directly queries pure concepts and type predicates; it does not implicitly apply a metafunction.
