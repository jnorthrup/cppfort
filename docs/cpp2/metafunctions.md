...
## <a id="pure-concepts-and-pure-metafunctions"></a> Pure concepts and pure metafunctions

Cpp2 distinguishes between two common compile-time facilities that may look similar on the surface but behave differently:

- Pure concepts: compile-time, boolean-valued predicates that can be used in `is` queries, `requires` clauses, and other places where you need a type or value to satisfy a constraint. They are purely a compile-time predicate: they do not generate code, apply defaults, or otherwise change the parse tree. They are typically implemented as library-level predicates and are lightweight to use. Examples include `std::integral` and `std::signed_integral`.

- Pure metafunctions: a class of metafunctions that, although written with the same `@` syntax and applied to definitions, are explicitly defined to be "pure" — they may assert constraints and perform compile-time checks, but they do not generate additional code or apply defaults. Pure metafunctions are useful when you want to attach semantic checks or naming conventions without affecting object layout, generated constructors, or virtualness.

Why this distinction matters
- Regular (impure) metafunctions can rewrite a declaration: they may add functions, change visibilities, or otherwise transform the parse tree. This is powerful for opt-in features that define behavior (e.g., `@value`, `@enum`, `@union`).
- Pure concepts and pure metafunctions do not rewrite the parse tree. They only validate or assert constraints. Using them keeps the declaration shape and generated code unchanged, and signals intent to the reader and the toolchain without modifying semantics or adding defaults.

How to use them
- Use a pure concept in `is` queries and `requires` constraints. For example, `x is std::integral` is a pure concept usage.
- Use a pure metafunction by applying it with `@` just like any other metafunction, but author it so that it performs only validation and side-effect-free checks. Example: `@contract` may perform invariant checks at compile-time without adding or changing members, while regular `@value` will generate members and defaults.

Design note: naming and intent
- Because both kinds are library-level constructs, libraries should clearly document whether a metafunction is "pure" or not. A pure metafunction is an opt-in `@`-annotation that asserts a constraint and maybe emits diagnostics but it won't alter the declaration's generated member functions, layout, or virtualness. A non-pure metafunction can be expected to transform the declaration in various ways.

Examples
- A pure concept used in a `requires` clause:

``` cpp
push_back: <T: type>
           (this, newValue: T)
           requires T is std::copyable
= {
    // behaves as usual — the concept here is only a compile-time constraint
}
```

- A pure metafunction that asserts some design property, but emits no generated defaults:

``` cpp
// Suppose we define @no_payload to assert that a type has no data members,
// but it does not add code to the type itself.
fst: @no_payload type = {
    // compile-time error if someone adds a data member here
}
```

Both pure concepts and pure metafunctions are first-class library constructs. Use them to make intent obvious and provide compile-time checks without modifying structure or generating defaults.
