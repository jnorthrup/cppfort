#line 2 "tests/test_self_parse.cpp2"
#include <string_view>
#include <vector>
#include <string>
#include <iostream>
...
#line 1 "tests/test_self_parse.cpp2"

#line 9 "tests/test_self_parse.cpp2"
namespace cpp2 {

#line 20 "tests/test_self_parse.cpp2"
class struct_event;
class lexer_event;
class pixel;
class bitmap;
...
#line 33 "tests/test_self_parse.cpp2"
class decl_boundary;
class tag_decl;
...
#line 261 "tests/test_self_parse.cpp2"
}
...
#line 1 "tests/test_self_parse.cpp2"
// test_self_parse.cpp2 — can cpp2.cpp2 parse its own source?

#line 9 "tests/test_self_parse.cpp2"
namespace cpp2 {

// Read file to string
[[nodiscard]] auto read_file(cpp2::impl::in<std::string> path) -> std::string;

#line 19 "tests/test_self_parse.cpp2"
// from stage0.cpp2 — types and functions (copied inline)
class struct_event {
...
#line 21 "tests/test_self_parse.cpp2"
};
...
#line 23 "tests/test_self_parse.cpp2"
class bitmap {
...
#line 32 "tests/test_self_parse.cpp2"
};
...
#line 34 "tests/test_self_parse.cpp2"
class tag_decl {public: std::string_view name {}; public: std::string_view type_name {}; public: int val {0}; 
public: tag_decl(auto&& name_, auto&& type_name_, auto&& val_)
CPP2_REQUIRES_ (std::is_convertible_v<CPP2_TYPEOF(name_), std::add_const_t<std::string_view>&> && std::is_convertible_v<CPP2_TYPEOF(type_name_), std::add_const_t<std::string_view>&> && std::is_convertible_v<CPP2_TYPEOF(val_), std::add_const_t<int>&>) ;
public: tag_decl();
};
#line 35 "tests/test_self_parse.cpp2"
...
#line 37 "tests/test_self_parse.cpp2"
...
#line 38 "tests/test_self_parse.cpp2"
...
#line 39 "tests/test_self_parse.cpp2"
class decl_kind {
...
#line 49 "tests/test_self_parse.cpp2"
};

// scanner functions (from stage0.cpp2)
[[nodiscard]] auto classify_se(cpp2::impl::in<char> c) -> struct_event;

#line 65 "tests/test_self_parse.cpp2"
[[nodiscard]] auto classify_le(cpp2::impl::in<char> c) -> lexer_event;

#line 74 "tests/test_self_parse.cpp2"
...
#line 12 "tests/test_self_parse.cpp2"
[[nodiscard]] auto read_file(cpp2::impl::in<std::string> path) -> std::string{
    std::ifstream f {path}; 
    std::stringstream buf {}; 
    buf << CPP2_UFCS(rdbuf)(cpp2::move(f));
    return CPP2_UFCS(str)(cpp2::move(buf)); 
}
...
requires (std::is_convertible_v<CPP2_TYPEOF(name_), std::add_const_t<std::string_view>&> && std::is_convertible_v<CPP2_TYPEOF(type_name_), std::add_const_t<std::string_view>&> && std::is_convertible_v<CPP2_TYPEOF(val_), std::add_const_t<int>&>) 
...
#line 52 "tests/test_self_parse.cpp2"
[[nodiscard]] auto classify_se(cpp2::impl::in<char> c) -> struct_event{
...
#line 75 "tests/test_self_parse.cpp2"
[[nodiscard]] auto encode(cpp2::impl::in<std::string_view> src) -> std::vector<pixel>{
    std::vector<pixel> r {}; 
...
[[nodiscard]] auto decode(cpp2::impl::in<std::string_view> src) -> bitmap{
    ...
    bitmap bmp {}; 
    bmp.src = src;
    auto in_string {false}; auto escape_next {false}; auto in_comment {false}; 
    auto string_idx {0}; auto comment_idx {0}; 
    for ( auto const& i : cpp2::range(0,CPP2_UFCS(ssize)(pixels)) ) {
        auto p {CPP2_ASSERT_IN_BOUNDS(pixels, i)}; 
        if (in_comment) {if (CPP2_ASSERT_IN_BOUNDS(src, i) == '\n') {in_comment = false; CPP2_UFCS(push_back)(bmp.comment_ends, i); }continue; }
        if (in_string) {
...
[[nodiscard]] auto index_decls(cpp2::impl::in<bitmap> bmp) -> std::vector<decl_boundary>{
    ...
    auto depth {0}; auto decl_start {0}; 
...
[[nodiscard]] auto find_at_depth(cpp2::impl::in<std::string_view> src, cpp2::impl::in<char> target, cpp2::impl::in<int> p) -> int{
    ...
    while( cpp2::impl::cmp_less(i,CPP2_UFCS(ssize)(src)) ) {
        ...
        if (in_string) {if (c == '\\') {i += 2; continue; }if (c == '"') {in_string = false; }i += 1; continue; }
...
[[nodiscard]] auto reify(cpp2::impl::in<std::string_view> src, cpp2::impl::in<decl_boundary> boundary) -> decl{
    ...
    p += cpp2::unchecked_narrow<int>(CPP2_UFCS(ssize)(name)); p = skip_ws(src, p);
...
#line 234 "tests/test_self_parse.cpp2"
[[nodiscard]] auto main() -> int{
    auto src {read_file("tests/stage0.cpp2")}; 
    std::cerr << "=== parsing stage0.cpp2 (" << CPP2_UFCS(ssize)(src) << " bytes) ===\n";

    auto decls {parse(cpp2::move(src))}; 
    std::cerr << "found " << CPP2_UFCS(ssize)(decls) << " declarations:\n";

    for ( auto const& i : cpp2::range(0,CPP2_UFCS(ssize)(decls)) ) {
        auto d {CPP2_ASSERT_IN_BOUNDS(decls, i)}; 
        auto kind_s {"unknown"}; 
        ...
        if (d.kind == decl_kind::tag_kind) {std::cerr << " " << cpp2::move(d).tag.name; }
        else {if (d.kind == decl_kind::ns_kind) {std::cerr << " " << cpp2::move(d).ns.name; }
        else {if (d.kind == decl_kind::type_kind) {std::cerr << " " << cpp2::move(d).tp.name; }
        else {if (d.kind == decl_kind::alias_kind) {std::cerr << " " << cpp2::move(d).al.name; }
        else {if (d.kind == decl_kind::func_kind) {std::cerr << " " << cpp2::move(d).fn.name; }}}}}
        std::cerr << "\n";
    }
