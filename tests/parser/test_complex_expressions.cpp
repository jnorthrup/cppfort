...
void check_emit(const std::string &input, const std::string &expected_contains,
...
int main() {
  ...
  check_emit("x: int = map[\"key\"];", "int x = map[\"key\"];",
             "Subscript string");
  check_emit("x: int = vec[i + 1];", "int x = vec[i + 1];",
             "Subscript expression");

  // Binary Operators
  check_emit("x: bool = a && b || c;", "bool x = a && b || c;", "Logical ops");
  check_emit("x: int = (a << 1) & 0xFF;", "int x = (a << 1) & 0xFF;",
             "Bitwise and shift");

  // Regression Debugging
  check_emit("x: int = 0xFF;", "int x = 0xFF;", "Hex literal decl");

  // Ternary Operator
  check_emit("x: int = cond ? 1 : 0;", "int x = cond ? 1 : 0;",
             "Ternary simple");
  check_emit("x: int = (a > b) ? a : b;", "int x = (a > b) ? a : b;",
             "Ternary expression");
