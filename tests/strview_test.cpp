...
auto main() -> int {
    std::string utf8_data = "He\xE2\x82\xAClo";
    cpp2::StrView sv = {std::data(utf8_data), std::size(utf8_data)};
    std::cout << "size: " << std::size(sv) << "\n";
    std::string padded = "  foo  ";
    cpp2::StrView sv_padded = {std::data(padded), std::size(padded)};
    auto trimmed = sv_padded.trim();
    std::cout << "trimmed size: " << std::size(trimmed) << "\n";
}
