...
// Enhanced with template context awareness for >> disambiguation

// Use the canonical `ConfixType` defined in `type_evidence.h` (do not re-define)
...
// Template context for >> disambiguation
...
public:
    ...
    static EvidenceSpan2D analyze_span(std::string_view text, size_t start_offset = 0) {
        ...
        for (size_t i = 0; i < text.length(); ++i) {
            ...
            // Handle structural confixes with template awareness
            ConfixType type = get_confix_type(c);
            if (type != ConfixType::INVALID) {
                // Special handling for >> in template context
                if (c == '>' && i + 1 < text.length() && text[i + 1] == '>') {
                    if (template_ctx.in_template && template_ctx.angle_depth >= 2) {
                        // Treat as two separate angle closes
