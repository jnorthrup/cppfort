...
double CPP2KeyResolver::compute_cpp2_similarity(const std::string& token_sequence,
    ...
    // Simple regex-based similarity for now
    // In production, this would use more sophisticated pattern matching

    try {
        std::smatch match;
        std::regex_search(token_sequence, match, key.compiled_pattern);

        if (match.empty()) {
            return 0.0;
        }

        // Calculate similarity based on match length vs total length
        double match_ratio = static_cast<double>(match.length()) / token_sequence.length();

        // Boost similarity for exact matches of key patterns
        if (match.length() == token_sequence.length()) {
            match_ratio *= 1.5;
        }
        ...
        std::cerr << "Regex error in CPP2 similarity computation: " << e.what() << std::endl;
        return 0.0;
    }
}
