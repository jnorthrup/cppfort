...
::std::string Emitter::emit_cpp(const TranslationUnit& unit, const EmitOptions& options) const {
    ...
    if (options.include_headers) {
        if (options.bundle_headers) {
            // Bundle cpp2.h - auto-generated, do not edit
            // Minimal cpp2 compatibility header (thin stubs for emitted code)
            output += "// Bundled cpp2.h - auto-generated, do not edit\n";
            output += "// Minimal cpp2 compatibility header (thin stubs for emitted code)\n";
            output += "#pragma once\n";
            output += "\n";
            ...
            output += "        using in = ::std::add_lvalue_reference_t<::std::add_const_t<T>>;\n";
            output += "\n";
            output += "        template<typename T>\n";
            output += "        using copy = T;\n";
            output += "\n";
            ...
            output += "\n";
            output += "        template<typename T>\n";
            output += "        using out = ::std::add_lvalue_reference_t<T>;\n";
            output += "\n";
            output += "        // forward helper (perfect-forwarding alias)\n";
            output += "        template<typename T>\n";
            output += "        using forward = T&&;\n";
            output += "\n";
            output += "        // simple assert-not-null helper\n";
            output += "        template<typename P>\n";
            output += "        inline P assert_not_null(P p) {\n";
            output += "            using Raw = ::std::remove_reference_t<P>;\n";
            output += "            if constexpr (::std::is_pointer_v<Raw>) {\n";
            output += "                if (!p) {\n";
            output += "                    ::std::cerr << \"cpp2: null pointer access\" << ::std::endl;\n";
            output += "                    static ::std::remove_pointer_t<Raw> default_value{};\n";
            output += "                    return &default_value;\n";
            output += "                }\n";
            output += "            }\n";
            output += "            return p;\n";
            output += "        }\n";
            output += "\n";
            output += "        template<typename P>\n";
            output += "        inline decltype(auto) deref(P p) {\n";
            output += "            return *assert_not_null(p);\n";
            output += "        }\n";
            output += "\n";
            output += "        // unchecked narrowing and casting helpers\n";
            output += "        template<typename To, typename From>\n";
            output += "        constexpr To unchecked_narrow(From v) {\n";
            output += "            return static_cast<To>(v);\n";
            output += "        }\n";
            output += "\n";
            output += "        template<typename To, typename From>\n";
            output += "        constexpr To unchecked_cast(From v) {\n";
            output += "            return reinterpret_cast<To>(v);\n";
            output += "        }\n";
            output += "\n";
            output += "        // simple type inspection / conversion helpers (very small)\n";
            output += "        template<typename To, typename From>\n";
            output += "        constexpr To as_(From&& v) {\n";
            output += "            return static_cast<To>(::std::forward<From>(v));\n";
            output += "        }\n";
            output += "\n";
            output += "        template<typename T, typename From>\n";
            output += "        constexpr bool is(From const&) noexcept {\n";
            output += "            // Conservative default: cannot determine dynamic types here.\n";
            output += "            return false;\n";
            output += "        }\n";
            output += "    } // namespace impl\n";
            output += "\n";
            output += "} // namespace cpp2\n";
            output += "\n";
            output += "// UFCS macro helper (captures function-like usage in generated code)\n";
            output += "#ifndef CPP2_UFCS\n";
            output += "#define CPP2_UFCS(X) (X)\n";
            output += "#endif\n";
            output += "\n";
            output += "using cpp2::impl::unchecked_narrow;\n";
            output += "using cpp2::impl::unchecked_cast;\n";
            output += "using cpp2::impl::assert_not_null;\n";
            output += "using cpp2::impl::as_;\n";
            output += "using cpp2::impl::is;\n";
            output += "\n";
        }
        ...
        if (options.inline_cpp2_header) {
            // Inline the cpp2.h contents instead of including it
            output += "\n// Inline cpp2.h contents\n";
            output += "namespace cpp2 {\n";
            output += "    using i8 = ::std::int8_t;\n";
            output += "    using i16 = ::std::int16_t;\n";
            output += "    using i32 = ::std::int32_t;\n";
