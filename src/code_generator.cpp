...
void CodeGenerator::generate_function_declaration(FunctionDeclaration* decl) {
    ...
    // Handle forward return: forward T → auto&&
    if (decl->is_forward_return && decl->return_type) {
        // Forward return of a type parameter becomes auto&&
        return_type = "auto&&";
    }
...
void CodeGenerator::generate_operator_declaration(OperatorDeclaration* decl) {
    ...
    // Handle subscript operator[], call operator(), and other non-assignment operators
    // These should be generated as regular member functions
    if (decl->name == "[]" || decl->name == "()") {
        ...
        for (const auto& param : decl->parameters) {
            ...
            std::string param_type = param->type ? generate_parameter_type(param->type.get(), param->qualifiers) : "auto";
            write(param_type + " " + param->name);
        }
