...
ParseResult Parser::parse(std::string_view source) {
    ...
    ast->set_source(std::string(source));
    source_ = ast->source();
    position_ = 0;
    current_line_ = 1;
    current_column_ = 1;
...
std::unique_ptr<ASTNode> Parser::parse_pipeline_expression() {
    auto left = parse_join_expression();
    if (!left) return nullptr;

    skip_whitespace();
    while (match("|>")) {
        auto right = parse_join_expression();
        if (!right) return left;
...
std::unique_ptr<ASTNode> Parser::parse_join_expression() {
    auto left = parse_ternary_expression();
    if (!left) return nullptr;

    while (true) {
        skip_whitespace();
        if (position_ >= source_.size() || source_[position_] != 'j') {
            break;
        }

        if (position_ + 1 < source_.size()) {
            const unsigned char next = static_cast<unsigned char>(source_[position_ + 1]);
        ...
        if (!right) {
            errors_.push_back(ParseError(current_line_, current_column_, "Expected rhs after join operator"));
            return left;
        }

        auto node = std::make_unique<ASTNode>(NodeTag::join_tag, "j");
        node->line = current_line_;
        node->column = current_column_;
        node->children.push_back(std::move(left));
        node->children.push_back(std::move(right));
