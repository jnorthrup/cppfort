...
// Note: This is a member function of Lexer as declared in header.
...
void Lexer::scan_token() {
    ...
    switch (c) {
        ...
        case '"': {
            ...
            if (start > 0 && (source[start - 1] == 'R' || (start > 1 && source[start - 1] == '"' && source[start - 2] == 'R'))) {
                scan_raw_string();
            } else if (start > 0 && (source[start - 1] == '$' || (start > 2 && source[start - 1] == 'R' && source[start - 2] == '$'))) {
                scan_raw_string();
            } else {
                scan_string();
            }
